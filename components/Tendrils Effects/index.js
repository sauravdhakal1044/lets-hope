...
export default function Tendrils() {
    ...
    function initOciliator(remove) {
        ...
        if (!remove) {
            ...
            function loop() {
                ...
                ctx.globalCompositeOperation = theme === "dark" ? 'lighter' : "darken";
                ctx.strokeStyle = 'hsla(346,98%,56%,0.25)';
                // ctx.strokeStyle = 'hsla(120,100%,50%,0.3)';
                ctx.lineWidth = 1;

                if (color === 1) {
                    ctx.strokeStyle = 'hsla(346,100%,56%,0.3)';
                } else {
    ...
    return (
        <>
            <canvas ref={canvasElement} id='canvas' className='canvas absolute h-full w-full'></canvas>
        </>
    )
}