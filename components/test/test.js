!(function ($, e, t, a) {
    ...
    ($.fn[i] = function (d) {
        ...
        function u(e, a) {
            ...
            if (
                ((r.className = i),
                a.customClass && ((r.className += " " + a.customClass), a.generateIndexID && (r.id = a.customClass + "-" + f.blastedIndex)),
        ...
        function p(t, s) {
            ...
            t
                ...
                s.debug && (n.log(i + ": Reversed Blast" + (t.attr("id") ? " on #" + t.attr("id") + "." : ".") + (l ? " Skipped reversal on the children of one or more descendant root elements." : "")), n.timeEnd("blast reversal"));
        }
        var m = $.extend({}, $.fn[i].defaults, d),
            h,
            f = {};
        if (m.search.length && ("string" == typeof m.search || /^\d/.test(parseFloat(m.search))))
            (m.delimiter = m.search.toString().replace(/[-[\]{,}(.)*+?|^$\\\/]/g, "\\$&")), (h = new RegExp("(?:^|[^-" + s.latinLetters + "])(" + m.delimiter + "('s)?)(?![-" + s.latinLetters + "])", "i"));
        else
            ...
            (this.each(function () {
                ...
                if (d !== !1) {
                    (f = { blastedIndex: 0, nodeBeginning: !1, wrappers: f.wrappers || [] }),
                        e.data(i) === a || ("search" === e.data(i) && m.search !== !1) || (p(e, m), m.debug && n.log(i + ": Removed element's existing Blast call.")),
